...
smolts <- subset(smolts, is.na(FL)==FALSE & is.na(WW)==FALSE)
smolts_clean <- subset(smolts, (FL<200 & WW < 5000) | (FL>200 & WW<40000) | FL==200)
ggplot() + geom_point(data=smolts_clean, aes(FL, WW))
...
vals <- data.frame(year.adj=rep(1955,4), scaledFL=rep(10.5, 4), RA=c(2,3,4,5),
...
plot1 + geom_text(data=vals, aes(year.adj, scaledFL, label=paste0("slope = ", round(slope,4), "\nadj. Rsq. = ", round(rsq,4))), size=3, hjust=0)
# 
# 
